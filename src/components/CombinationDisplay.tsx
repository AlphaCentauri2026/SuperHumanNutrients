...
export function CombinationDisplay({
  ...
  const handleShareCombination = async (
    combination: Combination,
    index: number,
    shareType: 'copy' | 'download' | 'social' = 'copy'
  ) => {
    try {
      const shareText =
        `${combination.name || `Combination ${index + 1}`}\n\n` +
        `🍎 Fruits: ${combination.fruits || 'None'}\n` +
        `🥕 Vegetables: ${combination.vegetables || 'None'}\n` +
        `🌾 Grain: ${combination.grain || 'None'}\n\n` +
        `💪 Benefits: ${combination.benefits || 'None'}\n\n` +
        `👨‍🍳 Preparation: ${combination.preparation || 'None'}\n\n` +
        `Generated by Superhuman Nutrition AI - super-human-nutrients.vercel.app`;

      if (shareType === 'copy') {
        await navigator.clipboard.writeText(shareText);
        setShareStatus({ ...shareStatus, [index]: 'copied' });

        // Clear success status after 3 seconds
        setTimeout(() => {
          setShareStatus(prev => ({ ...prev, [index]: null }));
        }, 3000);
        ...
        a.href = url;
        a.download = `${combination.name || `combination-${index + 1}`}.txt`;
      ...
      } else if (shareType === 'social') {
        const encodedText = encodeURIComponent(shareText);
        const twitterUrl = `https://twitter.com/intent/tweet?text=${encodedText}`;
        const facebookUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
          'https://super-human-nutrients.vercel.app'
        )}&quote=${encodedText}`;
        ...
        if (shareWindow) {
          shareWindow.document.write(`
            <html>
              <head><title>Share Combination</title></head>
              <body style="font-family: Arial, sans-serif; padding: 20px; text-align: center;">
                <h2>Share Your Nutrition Combination</h2>
                <div style="margin: 20px 0;">
                  <a href="${twitterUrl}" target="_blank" style="display: inline-block; margin: 10px; padding: 10px 20px; background: #1da1f2; color: white; text-decoration: none; border-radius: 5px;">Share on Twitter</a>
                  <a href="${facebookUrl}" target="_blank" style="display: inline-block; margin: 10px; padding: 10px 20px; background: #4267b2; color: white; text-decoration: none; border-radius: 5px;">Share on Facebook</a>
                </div>
                <p>Or copy the text below:</p>
                <textarea readonly style="width: 100%; height: 200px; margin: 10px 0;">${shareText}</textarea>
              </body>
            </html>
          `);
        }
      ...
      console.error('Failed to share combination:', err);
      setShareStatus({ ...shareStatus, [index]: 'error' });
  ...
  const toggleShareOptions = (index: number) => {
                ...
                >
                  {savedStatus[index] === 'saved' ? (
                    ...
                    <>
                      ...
                      {user ? 'Error - Try Again' : 'Login to Save'}
                    </>
                  ) : (
                    <>
                  ...
                  {showShareOptions[index] && (
                    <div className="absolute bottom-full left-0 right-0 mb-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-10">
                      <div className="p-2 space-y-1">
                        <button
                          onClick={() => {
                            handleShareCombination(combination, index, 'copy');
                            setShowShareOptions(prev => ({
                              ...prev,
                              [index]: false,
                            }));
